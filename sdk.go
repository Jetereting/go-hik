...
type HKConfig struct {
	...
	TagId   string // 自定义服务标识 eg:kingpark
}
...
// HttpPost
// @title		HTTP Post请求
// @url			HTTP接口Url		string		HTTP接口Url，不带协议和端口，如/api/resource/v1/org/advance/orgList
// @body		请求参数			interface
// @return		请求结果			参数类型
func (hk HKConfig) HttpPost(url string, body interface{}) (result gjson.Result, err error) {
	url = "/artemis" + url
	...
	client.Timeout = time.Duration(hk.Timeout) * time.Second
	if hk.IsHttps {
		tr := &http.Transport{
	...
	if resp.StatusCode == http.StatusOK {
		...
		if resg.Get("code").Str != "0" {
			err = fmt.Errorf("[error] hik：URL:%s resp：%s", url, ress)
			return
		}
